...
const strUser = 'newtypedemo@advantech.com.tw'		//Account
const strPasswod = 'newtypedemoP@ssw0rd'					//Password
const strPortal = 'e2i-demo.wise-paas.com'	//Portal
...
const strURL = 'ws://' + strPortal + '/event/' + strLanguage + '/' + btoa(strUser + ":" + strPasswod);
...
const receiveData = (strData) => {
    ...
    evtAry.map((item) => {
        console.log(item);
        //tranData(item);
        (item.severity == 'ERROR' && item.subtype == "NewType") ? tranData(item) : '';
    })
}
